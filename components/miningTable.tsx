...
export const MiningTable = ({ title = "Mining", description = "Recent mining activity" }: MiningTableProps) => {
	...
	return (
		<div className="w-full">
			<div className="text-[28px] font-bold text-[#fff] mb-[8px]">{t('Explore.mining')}</div>
			<div className="text-[14px] text-[#868789] mb-[16px]">{t('Explore.miningActivity')}</div>
						...
						{isLoading ? (
							...
							eventListInfo.list.map((row: any, index: any) => (
								<div key={index} className="flex min-h-[38px] items-center text-[12px] hover:bg-[#191B1F] transition-colors cursor-pointer px-[12px] rounded-[8px] py-[2px]">
									...
									<div className="w-[110px] lg:flex-[1.3] shrink-0 flex items-center justify-end gap-[4px] min-w-0">
										<BNBIcon className="w-[14px] h-[14px] shrink-0" />
										<span className="text-[#fff] truncate">{row?.vault_share ? BigNumber(ethers.formatUnits(BigInt(row.vault_share), 8)).dp(8).toString() : '0'}</span>
									</div>
									<div className="w-[110px] lg:flex-[1.3] shrink-0 flex items-center justify-end gap-[4px] min-w-0">
										<BNBIcon className="w-[14px] h-[14px] shrink-0" />
										<span className="text-[#fff] truncate">{row?.winnings ? BigNumber(ethers.formatUnits(BigInt(row.winnings), 8)).dp(8).toString() : '0'}</span>
									</div>
									<div className="w-[110px] lg:flex-[1.3] shrink-0 text-[#fff] break-words text-right">{row?.motherlode ? BigNumber(ethers.formatUnits(BigInt(row.motherlode_payout), 8)).dp(8).toString() : '0'}</div>
